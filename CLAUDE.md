...
- `_pending` is `@state` — drives the busy UI (button `disabled`, `loading` class, dots) and the re-entry guard; `updated()` reflects `aria-busy` onto the host.
